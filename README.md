...
> 📅 当前阶段: Phase 4+5 (AI模块+实盘交易) | AI预测/套利策略/策略筛选已上线
...
- 🎯 **多时间框架**: 日线重采样(周/月/N日)+多TF指标计算+MTF策略混入类
- 💼 **投资组合**: 等权/风险平价/固定权重资金分配+多品种组合回测聚合
- 💹 **实盘交易**: CTP/SimNow网关(模拟+实盘)+C++桥接DLL+订单/持仓/风险管理器
- 🔄 **断线重连**: CTP网关指数退避自动重连(TD/MD独立)，恢复登录/结算确认/行情订阅
- ⏰ **定时调度**: 开盘前检查→盘中健康检查(每30分钟)→收盘日报→定时数据采集
- 🎯 **参数优化**: 网格搜索+Walk-forward分析+综合评分排序+结果持久化
- 📊 **Web监控面板**: Streamlit 多页面面板，实时持仓/PnL曲线/日志/风控/手动干预
- 🤖 **AI预测**: 因子工程(Alpha101)+XGBoost/LightGBM+LSTM/Transformer+集成预测Pipeline
- 🔗 **套利策略**: 跨期/跨品种价差交易(Z-Score均值回归)+价差数据合成+回测引擎
- 📊 **策略筛选**: 多品种×多参数自动遍历，TopN榜单+参数稳定性分析+结果持久化
...
│   ├── portfolio/                    # ✅ 投资组合模块
│   │   ├── __init__.py               # ✅ 包入口
│   │   ├── allocator.py              # ✅ 资金分配(等权/风险平价/固定权重)
...
│   ├── web/                         # Web监控面板
│   │   ├── app.py                   # ✅ 主入口+侧边栏导航
│   │   ├── state_reader.py          # ✅ 统一状态读取(实时/快照双源)
│   │   ├── equity_recorder.py       # ✅ 权益历史记录器
│   │   └── pages/                   # ✅ 7个页面
│   │       ├── overview.py          #     总览仪表盘
│   │       ├── positions.py         #     持仓看板
│   │       ├── pnl.py               #     PnL曲线
...
│   ├── run_mtf_backtest.py           # ✅ MTF回测(多时间框架对比)
│   ├── run_portfolio_backtest.py     # ✅ 组合回测(多品种+分配策略)
│   ├── scheduler.py                  # ✅ 定时任务调度器
│   ├── optimize.py                   # ✅ 参数优化器(网格/Walk-forward)
│   ├── strategy_screener.py          # ✅ 策略自动筛选(TopN榜单+稳定性)
